...
class KalshiProvider(BaseProvider):
    ...
    def __init__(self, host: Optional[str] = None):
        self.host = host or os.getenv("KALSHI_API_HOST") or "https://api.kalshi.com/trade-api/v2"
    ...
    def _authenticate(self):
        ...
        import sys
        sys.stderr.write(f"[KALSHI AUTH] key_id={os.getenv('KALSHI_KEY_ID')}, path={os.getenv('KALSHI_PRIVATE_KEY_PATH')}\n")
        sys.stderr.flush()
        ...
        try:
            ...
            for e in raw_events:
                events.append(Event(
                    ...
                    status=MarketStatus.ACTIVE if e.get("status") == "open" else MarketStatus.CLOSED,
    ...
    async def get_markets(
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None, 
                lambda: self.api_instance.get_markets(**kwargs))
            ...
            for m in raw_markets:
                markets.append(Market(
                    ...
                    status=MarketStatus.ACTIVE if getattr(m, "status") == "open" else MarketStatus.CLOSED,
                    outcomes=["Yes", "No"],
    ...
    async def get_orderbook(self, market_id: str) -> OrderBook:
        ...
        try:
            ...
            for level in yes_bids:
                if isinstance(level, list): p, s = level[0], level[1]
                else: p, s = getattr(level, "price", 0), getattr(level, "count", 0)
            ...
            logger.error("Error fetching Kalshi orderbook", market_id=market_id, error=str(e))
            return OrderBook(market_id=market_id, bids=[], asks=[], timestamp=0)
    ...
    async def place_order(
        ...
        try:
            ...
            return Order(
                ...
                side=side,
                type=order_type,
                status=OrderStatus.OPEN if getattr(resp, "status") in ["placed", "submitted"] else OrderStatus.FILLED,
                timestamp=0.0
            )
        except Exception as e:
            logger.error("Kalshi order placement failed", market_id=market_id, error=str(e))
            raise
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Error cancelling Kalshi order", order_id=order_id, error=str(e))
            return False
        ...
        try:
            ...
            for p in raw_pos:
                ...
                positions.append(Position(
                    ...
                    size=float(count),
                    avg_price=float(getattr(p, "cost_basis", 0)) / (count * 100.0) if count else 0,
    ...
    async def get_history(self, market_id: Optional[str] = None) -> List[Trade]:
        ...
        if not self.api_instance or not market_id: return []
        try:
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(
                None,
