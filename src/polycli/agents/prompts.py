...
class Prompter:
    ...
    def sentiment_analyzer(self, question: str, outcome: str) -> str:
        ...
        You are given a question: {question} and an outcome of yes or no: {outcome}.
        
        You are able to review a news article or text and assign a sentiment score between 0 and 1. 
        """

    def prompts_market(self, data1: Any, data2: Any) -> str:
        current_market_data = str(data1)
        current_event_data = str(data2)
        return f"""
        You are an AI assistant for users of prediction markets (like Polymarket and Kalshi).
        Users want to place bets based on their beliefs of market outcomes such as political or sports events.

        Here is data for current markets: {current_market_data}
        and current events: {current_event_data}.
    ...
    def read_market(self) -> str:
        return "You are a prediction market analyst."

    def market_analyst_api(self) -> str:
        return """
        You are an AI assistant for analyzing prediction markets.
        You will be provided with JSON output for API data from prediction markets.
        Prediction markets let users bet on the outcome of future events in a wide range of topics, like sports, politics, and pop culture. 
    ...
    def superforecaster(self, question: str, description: str, outcomes: List[str]) -> str:
        ...
        Given these steps produce a statement on the probability of outcomes=`{outcome_str}` occurring.

        Give your response in the following format:

        I believe {question} has a likelihood `0.XX` for outcome of `OUTCOME_NAME`.
        """
    ...
    def one_best_trade(
        ...
        In your journey, you are committed to continuous learning, staying informed about the latest trends and developments in various sectors. Your emotional intelligence empowers you to remain composed under pressure, making rational decisions even when the stakes are high.
        Visualize yourself consistently achieving outstanding returns, earning recognition as the top trader. You inspire others with your success, setting new standards of excellence in the world of information markets.
