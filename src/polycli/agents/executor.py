...
class ExecutorAgent(BaseAgent):
    ...
    async def filter_events_with_rag(self, events: List[Any]) -> List[Tuple[Any, float]]:
        """Ported from reference: Use RAG to filter events"""
        if not self.prompter: return []
    ...
    async def source_best_trade(self, market_object: Any) -> str:
        ...
        # Adaptation: handle our internal models/metadata structure
    ...
    async def get_market_llm(self, user_input: str) -> str:
        ...
        if total_tokens <= self.token_limit:
            ...
            useful_keys = ['id','description','liquidity','outcomes','outcomePrices','volume','startDate','endDate','question','events']
